...
We provide a package for RTK Query code generation from OpenAPI schemas. It is published as `kahirokunn/kubernetes-typescript-client-codegen-openapi` and you can find the source code at [`packages/rtk-query-codegen-openapi`](https://github.com/reduxjs/redux-toolkit/tree/master/packages/rtk-query-codegen-openapi).
...
```bash
npx kahirokunn/kubernetes-typescript-client-codegen-openapi openapi-config.ts
```
...
import { generateEndpoints } from 'kahirokunn/kubernetes-typescript-client-codegen-openapi'
