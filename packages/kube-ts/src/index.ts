...
/**
 * Enforces `@kahirokunn/oazapfts-patched` to use the same TypeScript version as this module itself uses.
...
function enforceOazapftsTsVersion<T>(cb: () => T): T {
  const ozTsPath = require.resolve('typescript', { paths: [require.resolve('@kahirokunn/oazapfts-patched')] });
